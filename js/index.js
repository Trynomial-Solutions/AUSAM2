...
const NOT_FOUND_HTML = "<p><i><b>Could not find any info for this category. This may happen when you cut and paste from a PDF or web output rather than the HTML code generated by ADS. Please try again using the HTML code of the ADS web page.</b></i></p><p>&nbsp;</p><p>You can view a <a href='"+DEMO_VIDEO_URL+"'>how-to</a> here.</p><p>&nbsp;</p><p>If you think this is an error, please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a> with the entire text that you pasted in the box.</p>"
...
function modal_dialog(title, htmltext, mail_send=false) {
	if (mail_send!==false) {
		// send email notification to 3NS; mail_send has the data to send
...
//        return false;
	...
	if (subtext.length<8) {
		...
		$.ajax({
			...
			error: function( xhr, status, errorThrown ) {
				modal_dialog( "PubMed AJAX error", "Unexpected result. Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a>", xhr );
	...
	if (subtext.length<10) {
		...
		$.ajax({
			...
			error: function( xhr, status, errorThrown ) {
				modal_dialog( "License AJAX error", "Unexpected result. Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a>", xhr );
				console.log( "Error: " + errorThrown );
				console.log( "Status: " + status );
				console.dir( xhr );
			}
    ...
    //     },
    //     error: function( xhr, status, errorThrown ) {
    //         modal_dialog( "BoardCert AJAX error", "Unexpected result. Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a>", xhr );
    //         console.log( "Error: " + errorThrown );
    //         console.log( "Status: " + status );
    //         console.dir( xhr );
    //     }
...
function pmid_processed(rval) {
	...
	if (rval.pmid_count===0) {
		modal_dialog("No PMIDs found!", "No scholarly activity PMIDs were read in either the resident or faculty tables. <p>If you feel this is an error, please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a> with the entire text that you pasted in the box");
		return false;
	}

	if (rval.error.code!==0) {
		modal_dialog("Error processing PubMed", "Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a> with the entire text that you pasted in the box.<p>", rval);
		return false;
	}
...
function lic_processed(rval) {
	...
	if (rval.error.code!==0) {
		modal_dialog("Error processing License", "Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a> with the entire text that you pasted in the box.<p>", rval.error.text);
		return false;
	}
...
// 		}
// 		else {
// 			modal_dialog("Error processing BoardCert", "Please contact <a href='mailto:info@trynomialsolutions.com'>Trynomial Solutions</a> with the entire text that you pasted in the box.<p>", rval.error.text);
// 		}
// 		return false;
// 	}
